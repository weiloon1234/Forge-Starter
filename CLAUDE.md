...
- `make lint` includes `lint:starter`, which enforces the starter's DRY/SSOT guardrails for admin permission literals, shared test harness helpers, and shared admin page headers.
...
- Admin frontend permission literals live in exactly one place: `frontend/admin/src/permissions.ts`. Feature code imports typed constants from there instead of inlining permission keys.
- Standard admin pages use shared shells by default: `AdminDatatablePage` for datatable-backed pages, `AdminPageHeader` for non-datatable page headers.
- Integration-test harness helpers live in `tests/support/` only. New integration tests import them instead of duplicating boot/login/request helpers inline.
...
## FINAL NOTE: The rules and patterns in this document are not theoretical ideals — they codify the actual practices that have emerged from real-world experience building multiple projects on this starter. They are the distilled wisdom of "what works" in practice, not just "what sounds good" in theory. Following them is how you get the real benefits of the starter's architecture and discipline, and avoid the common pitfalls that arise when those patterns aren't followed.
