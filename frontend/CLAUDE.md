...
13. **Prefer canonical Tailwind classes over arbitrary values.** When a named class exists, use it — `font-black` not `font-[900]`, `wrap-anywhere` not `[overflow-wrap:anywhere]`, `gap-2` not `gap-[0.5rem]`, `text-sm` not `text-[0.875rem]`. The Tailwind language server flags these as `suggestCanonicalClasses` warnings. Arbitrary `[...]` value syntax is the escape hatch ONLY for values genuinely outside the design tokens (one-off magic numbers from a Figma spec, dynamic runtime values). Common canonicals worth memorizing: weights `font-thin/light/normal/medium/semibold/bold/extrabold/black` (100–900), wrapping `wrap-anywhere` / `wrap-break-word` / `wrap-normal`, and the full named spacing / sizing / text scale. Before writing `[anything]`, check whether Tailwind already has the token. Drift here erodes the `@theme` SSOT in `styles/app.css` because arbitrary values can't pick up future token overrides.
